...
app.post('/backend/upload', photosMiddleware.array('photos', 100), async (req,res) => {
  const uploadedFiles = [];
...
app.post('/backend/places', (req,res) => {
  mongoose.connect(process.env.MONGO_URL);
  const {token} = req.cookies;
  const {
...
app.post('/backend/bookings', async (req, res) => {
  mongoose.connect(process.env.MONGO_URL);
...
app.get('/backend/bookings', async (req,res) => {
  mongoose.connect(process.env.MONGO_URL);
  const userData = await getUserDataFromReq(req);
